// src/data/number_of_services_per_region.js
...
async function getAllRegionCodes(ssm) {
  ...
  do {
    ...
    for (const param of resp.Parameters) {
      // The region code is the last part of the parameter name
...
async function getServicesForRegion(ssm, regionCode) {
  ...
  do {
    ...
    for (const param of resp.Parameters) {
      // The service name is the last part of the parameter name
...
async function main() {
  ...
  try {
    ...
    // Print CSV header
    console.log("regioncode,count");

    for (const regionCode of regionCodes) {
      const services = await getServicesForRegion(ssm, regionCode);
      console.log(`${regionCode},${services.length}`);
    }
